...
int video_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        SDL_Rect rcWindow;
        SDL_GetWindowPosition(screen, &rcWindow.x, &rcWindow.y);
        SDL_GetWindowSize(screen, &rcWindow.w, &rcWindow.h);
        ...
        av_usleep(p_codec_ctx->framerate.den * 1000 / p_codec_ctx->framerate.num);
    }
...
int _tmain(int argc, char *argv[])
{
    ...
    ret = avformat_open_input(&p_fmt_ctx, "F:\\Git\\FFmpegVideoPlayer\\bin\\Debug\\bird.mov", NULL, NULL);
    if (ret != 0)
    {
        av_make_error_string(szErr, 128, ret);
