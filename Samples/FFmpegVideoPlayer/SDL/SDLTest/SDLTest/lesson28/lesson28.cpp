...
bool init()
{
    ...
    win = SDL_CreateWindow("My Game Window",
    ...
    SDL_SetWindowTitle(win, "Particle Test" );
...
bool load_files()
{
    ...
    //Set alpha
    SDL_SetSurfaceAlphaMod( red, 192 );
    SDL_SetSurfaceAlphaMod( blue, 192 );
    SDL_SetSurfaceAlphaMod( green, 192 );
    SDL_SetSurfaceAlphaMod( shimmer, 192 );
    ...
    _In_ int       nCmdShow)
{
    ...
    while( quit == false )
    {
        ...
        //Update the screen
        if(SDL_UpdateWindowSurface( win ) == -1 )
        {
            return 1;
        }
